...
@baseline_registry.register_trainer(name="phase2_data_collector")
class Phase2DataCollector(BaseVLNCETrainer):
    def __init__(self, config=None):
        ...
        self.img_model = BlipForConditionalGeneration.from_pretrained("Salesforce/blip-image-captioning-large").to("cuda")

        
        self.prompt = "A room with "
        ...
        with lmdb.open(self.lmdb_features_dir,map_size=int(self.config.IL.DAGGER.lmdb_map_size),) as lmdb_env, torch.no_grad():
            ...
            while collected_eps < remain_update_size and not (lmdb_env.stat()["entries"] >= required_size):
                ...
                for i in range(self.envs.num_envs):
                    ...
                    # raw_img.save(f"{self.world_rank}_{self.timesteps[i]}_{self.processor.decode(out[0], skip_special_tokens=True)}.jpg", "JPEG")
                    # if self.timesteps[0] == 5:
                    #     assert 1==2
