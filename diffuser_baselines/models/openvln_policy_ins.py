...
class OpenVLNPolicyIns(NetPolicy):
    ...
    def __init__(
        ...
        self.vlm = load(base_vlm, hf_token=hf_token, load_for_training=True, flash_atten = config.OPENVLN.flash_atten,load_type="ins")
    ...
    def build_loss(self,observations):
        ...
        for sample in range(len(collected_data['ins_text'])):
            ...
            collected_data['ins_text'][sample] = self.tokenlizer(prompt_text, truncation=False, return_tensors="pt").input_ids[0] # auto added BOS , in shape (T)
            ori_instructions.append(self.tokenlizer(ori_ins, truncation=False, return_tensors="pt").input_ids[0])
        ...
        ori_instructions = pad_sequence(ori_instructions, batch_first=True, padding_value=self.tokenlizer.pad_token_id).to(observations['instruction'].device)
        ori_mask = ori_instructions.ne(self.tokenlizer.pad_token_id)
...
class OpenVLN(PrismaticVLM):
    ...
    def compress_memories(self,projected_cls_embeddings,img_ori_shape,multimodal_embeddings,pre_mask,ori_ins): # grid pooled version
        ...
        # compressing his & ins
        language_filtered_his,_ = self.memory_fuser_attention(query=original_instructions.transpose(0, 1),
            value=his_pos.transpose(0, 1),
            query_pos=None,
            value_pos=None,
            diff_ts=None,pad_mask=pre_mask,print_info=False)
        language_filtered_his = language_filtered_his[-1].transpose(0,1) # (bs*T,C,d)


        compressed_memory,_ = self.memory_fuser_attention_sec(query=expanded_memory.transpose(0, 1),
            value=language_filtered_his.transpose(0, 1),
            query_pos=None,
            value_pos=None,
            diff_ts=None,pad_mask=original_instructions_mask,print_info=False)
    ...
    def forward(
        ...
        compressed_memory = self.compress_memories(projected_cls_embeddings,img_ori_shape,multimodal_embeddings,pre_mask,ori_inst)
