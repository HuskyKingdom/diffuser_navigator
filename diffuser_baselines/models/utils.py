...
# Phi LLM Adapte-Classes

from transformers import PhiForCausalLM
from transformers.models.phi.modeling_phi import PhiModel, PhiDecoderLayer, PhiFlashAttention2, PhiSdpaAttention


class MemoryPhiFlashAttention2(PhiFlashAttention2):
    """
...
class MemoryPhiSdpaAttention(PhiSdpaAttention):
    def __init__(self, config,layer_idx):
        super().__init__(config,layer_idx)
        self.k_mem_proj = nn.Linear(self.hidden_size, self.num_key_value_heads * self.head_dim, bias=True)
        self.v_mem_proj = nn.Linear(self.hidden_size, self.num_key_value_heads * self.head_dim, bias=True)
    ...
    def forward(
        ...
        mem_len = compressed_mem.shape[1]
        mem_key_states = self.k_mem_proj(compressed_mem)
        mem_value_states = self.v_mem_proj(compressed_mem).view(bsz, mem_len, self.num_key_value_heads, self.head_dim).transpose(1, 2)
        ...
        mem_key_states = mem_key_states.contiguous()
        mem_value_states = mem_value_states.contiguous()
        ...
        mem_attn_output = mem_attn_output.transpose(1, 2).contiguous()
        mem_attn_output = mem_attn_output.reshape(bsz, q_len, self.hidden_size)
...
class MemoryPhiDecoderLayer(PhiDecoderLayer):

    def __init__(self, config, layer_idx):
        super().__init__(config, layer_idx)

        if isinstance(self.self_attn,PhiSdpaAttention):
            self.self_attn = MemoryPhiSdpaAttention(config=config, layer_idx=layer_idx)
        else:
            self.self_attn = MemoryPhiFlashAttention2(config=config, layer_idx=layer_idx)
...
class MemoryPhiForCausalLM(PhiForCausalLM):
    _tied_weights_keys = ["lm_head.weight"]

    # Copied from transformers.models.llama.modeling_llama.LlamaForCausalLM.__init__ with Llama->Phi,bias=False->bias=True
    def __init__(self, config):
        super().__init__(config)
        self.model = MemoryPhiModel(config)
