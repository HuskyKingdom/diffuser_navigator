...
class DiffusionNavigator(nn.Module):
    ...
    def forward(self, observations, run_inference=False):
        ...
        tokens = self.tokenlize_input(observations,noised_traj)
        tokens = (tokens[0].unsqueeze(0),tokens[1].unsqueeze(0),tokens[2].unsqueeze(0),tokens[3].unsqueeze(0),tokens[4].unsqueeze(0))
    ...
    def inference_actions(self,tokens,mask): # pred_noises (B,N,D)
        ...
        for t in timesteps:
            ...
            with torch.no_grad():
                ...
                pred_noises = self.predict_noise(tokens,encoded_noise_embedding,t * torch.ones(len(tokens[0])).to(tokens[0].device).long(),mask)
